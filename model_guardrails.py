...
class RepeatNGramPenalty:
    ...
    def score(self, sources, generateds, **kwargs):
        ...
        for generated in generateds:
            ...
            n_grams = [
                tuple(words[i : (i + self.gram)])
                for i in range(len(words) - self.gram + 1)
            ]
            ...
            if not self.keep_stop_ws:
                n_grams = [
                    ...
                    if any(w not in self.stop_words for w in ngram)
                ]

            n_repeated_three_grams = len(
                [ngram for ngram, count in Counter(n_grams).most_common() if count > 1]
            )
...
def doc2ents(doc, black_list_types=[]):
    ents = [
        {"type": ent.label_, "entity": ent.text, "sent_idx": sent_idx}
...
class NERInaccuracyPenalty:
    def __init__(self, spacy_model="en_core_web_sm"):
        ...
        self.word2num = {}
        self.black_list_types = set(
            ["ORDINAL", "WORK_OF_ART", "EVENT", "PRODUCT", "LAW", "LANGUAGE"]
        )
        self.number_words_to_remove = set(
            [
                "the",
                "a",
                "an",
    ...
    def quantifier_cleaning(self, quantifier_text):
        ...
        words = sorted(
            [w for w in words if len(w) >= 2 and w not in self.number_words_to_remove]
        )
        return set(words)
    ...
    def quantifier_matching(self, quantifier, entity_list):
        ...
        entity_list_clean = [
            self.quantifier_cleaning(ent["text"]) for ent in entity_list
        ]
    ...
    def remove_common_entities(self, ent_list_new, ent_list_old, source_text):
        ...
        for ent_new in ent_list_new:
            ...
            if self.common_ents_no_problem(
                entity_text
            ):  # The entity is too common and could added anywhere
                continue
            if (
                entity_text in ent_set or self.singular(entity_text) in ent_set
            ):  # Exact match with some entity
                continue
            if (
                entity_text in source_text
                or self.singular(entity_text).lower() in source_text
                or raw_entity_lower in source_text
            ):
            ...
            if ent_new["type"] in ["DATE", "CARDINAL", "MONEY", "PERCENT"]:
                ...
                if ent_new["type"] == "CARDINAL":
                    if (
                        raw_entity_lower in self.string2digits
                        and self.string2digits[raw_entity_lower] in source_text
                    ):
                        continue  # They wrote "nineteen" instead of 19
                    elif raw_entity_lower in self.digits2string and self.digits2string[
                        raw_entity_lower
    ...
    def score(self, sources, generateds, printing=False, **kwargs):
        ...
        scores, all_new_ents = [], []
        for source_ent, generated_ent, source in zip(
            source_ents, generated_ents, sources
        ):
        ...
        return {
            ...
            "gen_ents": generated_ents,
            "new_ents": all_new_ents,
        }
