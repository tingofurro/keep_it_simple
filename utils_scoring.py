...
class ScorerWrapper:
    def __init__(
        self, scorers, scoring_method="logsum", max_batch_size=100, use_caching=False
    ):
        assert scoring_method in [
            "sum",
            "product",
            "logsum",
        ], "Unrecognized `scoring_method`"
    ...
    def score(
        ...
        if self.use_caching and len(todo) < len(all_keys):
            print(
                "With caching, only processing: %d / %d samples"
                % (len(todo), len(all_keys))
            )

        if len(todo) == 0:
            progress = False  # Not needed, it's empty

        for batch_todo in utils_misc.batcher(
            todo, batch_size=self.max_batch_size, progress=progress
        ):
