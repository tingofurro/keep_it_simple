...
class Generator:
    ...
    def reload(self, from_file, strict=True):
        ...
        loaded_dict = {
            k.replace("module.module.", ""): v for k, v in loaded_dict.items()
        }
    ...
    def preprocess_batch(self, encoded, decoded):
        ...
        decs = [
            dec[: (self.max_output_length - 1)] for dec in decs
        ]  # We cut short, but we want the end token at the end

        decs_inp = pad(
            [torch.LongTensor([self.start_id] + dec) for dec in decs], padval=0
        ).to(self.device)
        decs_out = pad(
            [torch.LongTensor(dec + [self.tokenizer.eos_token_id]) for dec in decs],
            padval=-1,
        ).to(self.device)
    ...
    def train_batch(
        ...
        assert (
            decoded is not None or decoded_tokenized is not None
        ), "Train batch should either receive decoded (list of text), or decoded_tokenized (list of list of token integers)."

        if decoded_tokenized is not None:
            # We are forcing to re-use pre-tokenized stuff
            encs = self.preprocess_input(encoded)
            decs_inp = pad(
                [torch.LongTensor([self.start_id] + dec) for dec in decoded_tokenized],
                padval=0,
            ).to(self.device)
            decs_out = pad(
                [
                    torch.LongTensor(dec + [self.tokenizer.eos_token_id])
                    for dec in decoded_tokenized
                ],
                padval=-1,
            ).to(self.device)
        else:
    ...
    def generate_ckpt_batch(
        ...
        **kwargs
        ...
        assert (
            top_p == 1.0
        ), "For now, the top_p implementation does not work, as the sampling on GPU will crash randomly"
        N = len(encoded_texts) * ckpt_runs

        # Used in the checkpointing
        encoded_toks = [
            self.tokenizer.encode(encoded_text, add_special_tokens=False)
            for encoded_text in encoded_texts
        ]
        encoding_equivalents = [
            self.tokenizer.decode(encoded_tok)
            for encoded_tok in encoded_toks
            for _ in range(ckpt_runs)
        ]  # The unlooped version
        ...
        while build_up.shape[1] < max_output_length and not finished_func(build_up):
            ...
            logits = utils_sampling.ngram_copy_filtering(
                build_up, inputs_repeated, logits, n_gram=no_copy_ngram
            )
            logits = utils_sampling.ngram_copy_filtering(
                build_up, build_up, logits, n_gram=no_repeat_ngram
            )
            ...
            if (build_up.shape[1] - 1) % ckpt_every == 0:
                ...
                chosen_tracks = (
                    torch.arange(0, N, step=ckpt_runs).cuda() + best_idx_in_each
                )
                chosen_tracks = torch.repeat_interleave(
                    chosen_tracks, repeats=ckpt_runs, dim=0
                )
        ...
        encodings_repeated = [
            encoded_text for encoded_text in encoded_texts for _ in range(ckpt_runs)
        ]
        ...
        chosen_tracks = torch.arange(0, N, step=ckpt_runs).cuda() + torch.argmax(
            folded_scores, dim=-1
        )
        ...
        outputs_list = [
            {k: outputs[k][i] for k in outputs} for i in range(len(encoded_texts))
        ]
        return outputs_list
    ...
    def past_repeat_interleave(self, past, beam_size):
        if not self.seq2seq:
            ...
            past["attention_mask"] = torch.repeat_interleave(
                past["attention_mask"], repeats=beam_size, dim=0
            )
            past["encoder_outputs"] = tuple(
                [
                    torch.repeat_interleave(X, repeats=beam_size, dim=0)
                    for X in past["encoder_outputs"]
                ]
            )
            return past
    ...
    def generate_beam_batch(
        ...
        **kwargs
        ...
        one_every_k = (
            torch.FloatTensor([1] + [0] * (beam_size - 1))
            .repeat(batch_size * beam_size)
            .to(self.device)
        )
        ...
        while build_up.shape[1] < max_output_length and not finished_func(build_up):
            ...
            if is_force_start:
                ...
                all_selects = torch.cat(
                    [distrib.sample().unsqueeze(-1) for _ in range(beam_size)], dim=1
                )
            ...
            expanded_not_finished = torch.repeat_interleave(
                not_finished, repeats=beam_size
            )

            expanded_logprobs = torch.repeat_interleave(
                seq_logprobs, repeats=beam_size
            )  # This should be batch_size * beam_size²
            expanded_logprobs += (
                expanded_not_finished
                * logprobs[
                    torch.repeat_interleave(torch.arange(N), repeats=beam_size),
                    all_selects.view(-1),
                ]
            )
            ...
            expanded_logprobs -= (
                (1 - expanded_not_finished) * (1 - one_every_k) * 1000.0
            )
            ...
            if (
                build_up.shape[1] == 1
                or (len(force_start_ids) == build_up.shape[1] - 1)
                or next_force_split
            ):
                ...
                _, batched_choices = torch.topk(
                    batched_scores, k=beam_size, dim=1
                )  # Going from k² choices per element to k choices.
            ...
            ).to(self.device) + batched_tracks.view(-1)

            selected_scores = batched_scores[
                torch.repeat_interleave(torch.arange(batch_size), repeats=beam_size),
                batched_choices.view(-1),
            ]
            selected_logprobs = batched_logprobs[
                torch.repeat_interleave(torch.arange(batch_size), repeats=beam_size),
                batched_choices.view(-1),
            ]
            ...
            next_words = (
                next_words * not_finished.view(-1, 1).long()
            )  # + (1-not_finished.view(-1, 1).long()) * end_id # This is so that nothing gets written past end_token but other end_tokens
            ...
            if ckpt_every > 0 and (build_up.shape[1] - 1) % ckpt_every == 0:
                ...
                so_far_scores = scorer(
                    expanded_inputs, generated_so_far, partial=True, printing=printing
                )
                so_far_scores = torch.FloatTensor(so_far_scores["total_scores"]).cuda()
                ...
                best_idx_in_each = torch.argmax(folded_scores, dim=-1)
                chosen_tracks = (
                    torch.arange(0, N, step=beam_size).cuda() + best_idx_in_each
                )
                chosen_tracks = torch.repeat_interleave(
                    chosen_tracks, repeats=beam_size, dim=0
                )
        ...
        for orig_beams, beam_logprobs, beam_scores in zip(
            batched_build_up, batched_logprobs, batched_scores
        ):
                ...
                [
                    {
                        "output_text": out_txt,
                        "output_tokens": beam,
                        "orig_output_tokens": orig_beam.tolist(),
                        "logprob": lp.item(),
                        "score": score.item(),
                    }
                    for out_txt, beam, orig_beam, lp, score in zip(
                        output_txts, beams, orig_beams, beam_logprobs, beam_scores
                    )
                ]
            )
    ...
    def generate(
        ...
        **kwargs
        ...
        assert not (
            beam_size > 1 and ckpt_runs > 1
        ), "Cannot ask for beam search and ckpt generation at the same time"
        if ckpt_runs > 1:
            assert (
                "ckpt_every" in kwargs and "scorer" in kwargs
            ), "Required parameters were not fed to the generation function."
        ...
        for i in iterator:
            ...
            with torch.no_grad():
                if beam_size > 1:
                    ...
                    batch_outputs = self.generate_beam_batch(
                        batch_bodies, beam_size=beam_size, **kwargs
                    )
                elif ckpt_runs > 1:
                    # print("Will run generate ckpt batch")
                    batch_outputs = self.generate_ckpt_batch(
                        batch_bodies, ckpt_runs=ckpt_runs, **kwargs
                    )
                else:
...
if __name__ == "__main__":
    ...
    from model_guardrails import (
        SimplificationRShift,
        SimplificationVShift,
    ...
    coverage_scorer = CoverageModel(
        KeywordMasker(mask_ratio=0.4),
    ...
    scorers = [
        {
            "name": "vshift",
            "model": SimplificationVShift(
                target_shift=vshift_target, word_change_ratio=word_change_ratio
            ),
            "sign": 1,
            "weight": 2.0,
        },
    ...
    scorer = utils_scoring.ScorerWrapper(
        scorers, scoring_method="logsum", max_batch_size=12
    )
    ...
    preds = model.generate(
        [ori_paragraph], num_runs=8, sample=True, sort_score=True, scorer=scorer
    )[0]
    ...
    for i, pred in enumerate(preds):
        ...
        print("---")
        print(
            utils_edits.show_diff_word(
                ori_paragraph, pred["output_text"], is_latex=True
            )
        )
