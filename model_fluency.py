...
class FluencyRelativeScore:
    ...
    def preprocess_batch(self, decoded):
        ...
        decs_inp = torch.nn.utils.rnn.pad_sequence(
            [torch.LongTensor([self.tokenizer.bos_token_id] + dec) for dec in decs],
            batch_first=True,
            padding_value=0,
        )
        decs_out = torch.nn.utils.rnn.pad_sequence(
...
class TextDiscriminator:
    ...
    def train_from_dataset(self, texts, labels, n_epochs=1):
        ...
        d_train, d_dev = torch.utils.data.dataset.random_split(
            dataset, [N_train, N_dev]
        )
        print(
            "Num train: %d, num dev: %d; Label Count: %s"
            % (len(d_train), len(d_dev), str(Counter(labels)))
        )

        train_sampler, dev_sampler = RandomSampler(d_train), RandomSampler(d_dev)

        train_dataloader = DataLoader(
            d_train, sampler=train_sampler, batch_size=train_batch_size
        )
        ...
        self.discriminator = RobertaForSequenceClassification.from_pretrained(
            "roberta-base"
        ).to("cuda")
        ...
        imbalance_weight = torch.FloatTensor(
            [len(labels) / label_counter[0], len(labels) / label_counter[1]]
        ).cuda()
        if self.fp16:
        ...
        for _ in range(n_epochs):
            ...
            for i, batch in enumerate(tqdm.tqdm(train_dataloader)):
                ...
                with autocast(self.fp16):
                    model_outputs = self.discriminator(
                        batch_inputs
                    )  # , labels=batch_labels
            ...
            with torch.no_grad():
                ...
                if val_f1 >= best_f1:
                    ...
                    best_f1 = val_f1
                print(
                    "Discriminator Validation. [Acc: %.3f] [F-1: %.3f]"
                    % (val_accuracy, val_f1)
                )
        ...
        with torch.no_grad():
            for batch in dev_dataloader:
                batch_inputs, batch_labels = tuple(t.to("cuda") for t in batch)
                model_outputs = self.discriminator(batch_inputs)
                preds_probs = torch.nn.functional.softmax(
        ...
        print(
            "[Final Discriminator] [Accuracy: %.3f] [F1: %.3f] [Average prediction: %.3f]"
            % (val_accuracy, val_f1, np.mean(total_pred_1s))
        )
    ...
    def score(self, sources, generateds, partial=False, printing=False, **kwargs):
        ...
        if (
            len(set(self.cache_generateds) | set(self.cache_sources))
            >= self.retrain_every
        ):
            self.retrain_auto()
        ...
        if not self.trained:
            ...
            generateds = [
                text if len(text) > 0 else "empty text" for text in generateds
            ]  # Trying to fix the empty sequence problem
            toks = [
                torch.LongTensor(self.tokenizer.encode(text))[:200]
                for text in generateds
            ]
            toks = [
                tok if len(tok) > 0 else [1] for tok in toks
            ]  # Make sure the sequence length is not zero, otherwise it crashes
