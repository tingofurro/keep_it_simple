...
def build_up2edit(build_up):
    ...
    if len(greens) > 0 and len(reds) > 0:
        return {
            "type": "substitution",
            "insertion": to_words(greens),
            "deletion": to_words(reds),
            "idx": idx1,
            "end_idx": idx1 + len(reds),
        }
    elif len(greens) > 0:
        return {
            "type": "insertion",
            ...
            "idx": idx1,
            "end_idx": idx1,
        }  # +1 For now removing the +1 seeing what happens
    elif len(reds) > 0:
        return {
            "type": "deletion",
            "deletion": to_words(reds),
...
def show_diff_word(
    text1, text2, is_HTML=False, is_latex=False, print_red=True, print_green=True
):
    ...
    while i < len(w1s):
        if i in idx2edit:
            ...
            del idx2edit[
                i
            ]  # Delete it so we don't loop on insertions, only apply it once.
            i = edit["end_idx"]
        else:
